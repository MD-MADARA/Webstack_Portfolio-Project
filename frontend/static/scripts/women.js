...
caregories.forEach(category => {
  fetch(`http://127.0.0.1:5001/api/products?category_type=Women&order_desc=id&category_name=${category}&limit=5`)
  .then(response => {
    if (!response.ok) {
...
function handleRadioClick(event) {
  ...
  fetch(`http://127.0.0.1:5001/api/products?category_type=Women&order_desc=id&category_name=${category_name}`)
  .then(response => {
    if (!response.ok) {
      throw new Error('Network response was not ok');
    }
